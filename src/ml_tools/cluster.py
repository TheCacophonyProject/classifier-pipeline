...
def main():
    ...
    run_umap(args.model,args.output.with_name(f"{args.output.stem}-features.npy"),new_labels,filter_labels)
    return
...
def run_umap(model_file,features_file,labels,filter_labels=None):
    ...
    tracks_file = features_file.with_name(f"{features_file.stem.replace("-features","-tracks")}.npy")
    ...
    anomalies_file = features_file.with_name(f"{features_file.stem.replace('-features', '-anomalies')}.json")
