...
def parse_args():
    ...
    parser.add_argument("isoforest", type=Path, help="iso forestmodel file")

    parser.add_argument("source", type=Path, help="cptv file with txt")
    args = parser.parse_args()
    args.model = Path(args.model)
    args.source = Path(args.source)
    args.isoforest = Path(args.isoforest)
    return args
...
def main():
    ...
    # 3. Reload the model in a new session or isolation
    iso_forest = joblib.load(args.isoforest)
