...
def main():
    ...
    model = ModelCRNN_HQ(labels=len(labels), enable_flow=True, l2_reg=0.0, keep_prob=0.2)
    ...
    model.train_model(epochs=30, run_name='HQ Novelty/V1 updated settings/'+datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
    model.save()
    model.close()
