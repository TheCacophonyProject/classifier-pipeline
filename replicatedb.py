...
def copy(old_db,new_db):
    ...
    for clip_id in clips:
        ...
        for k,v in og_frames.items():
            ...
            copy_frame = copy_og.create_dataset(str(k), shape = v.shape, chunks = v.shape,**gzip_compression,dtype = v.dtype )
            copy_frame[:] = v
        # break
    copy.close()
    f.close()
def copy_group(f,copy,key,og_frames, upper=False ):
        ...
        for g in group:
            ...
            if isinstance(group[g], h5py.Group):
                ...
                copy_ds = copy_g.create_dataset(g, shape = ds.shape, chunks = ds.chunks,**gzip_compression,dtype = ds.dtype )
                copy_ds[:] = ds
